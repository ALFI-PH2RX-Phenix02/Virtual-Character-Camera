...
public class FaceDetector {
    ...
    private Context orig_context;
    private CharacterView orig_characterView;
    private FaceDet mFaceDet;
    private Paint mLandmarkPaint;

    private final double r_m = 0.4, r_n = 0.5;

    public FaceDetector(Context c, CharacterView activity) {
        this.orig_context = c;
        this.orig_characterView = activity;
    ...
    public void getLandmarks(Canvas canvas, Bitmap bmp) {
        ...
        //synchronized (this) {
            ...
            //mark only the first face detected
            ...
            solveEmotion(landmarks, ret.getRight() - ret.getLeft());
        //}
    }


    //calculate the position and angle of face
    private void solveFacePose(ArrayList<Point> landmarks) {
        ...
        this.orig_characterView.emotion[0] = yaw;
        this.orig_characterView.emotion[1] = pitch;
        this.orig_characterView.emotion[2] = roll;
    }

    private void solveEmotion(ArrayList<Point> landmarks, int width) {
        //mouth open
        if(landmarks == null || landmarks.size() == 0) return;
        this.orig_characterView.emotion[3] = (double)(landmarks.get(57).y-landmarks.get(51).y) /
